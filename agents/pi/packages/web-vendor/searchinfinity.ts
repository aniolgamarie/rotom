...
const agentcfgFetch = webFetchFor("searchinfinity");
...
export function isSearchinfinityAvailable(): boolean {
	if (!agentcfgServiceSelected("searchinfinity")) return false;
...
async function searchinfinityJsonRequest(
	...
	try {
		response = await agentcfgFetch(SEARCHINFINITY_SEARCH_URL, {
			method: "POST",
			headers: {
