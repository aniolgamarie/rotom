...
const agentcfgFetch = webFetchFor("valyu");
...
export function isValyuAvailable(): boolean {
	if (!agentcfgServiceSelected("valyu")) return false;
	return hasCredentialSource({ provider: "Valyu", configuredValue: loadConfig().valyuApiKey, environmentValue: undefined });
}
...
export async function searchWithValyu(query: string, options: SearchOptions & { includeContent?: boolean } = {}): Promise<SearchResponse> {
	...
	try {
		response = await agentcfgFetch(VALYU_SEARCH_URL, {
			method: "POST",
