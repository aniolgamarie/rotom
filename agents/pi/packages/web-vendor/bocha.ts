...
const agentcfgFetch = webFetchFor("bocha");
...
export function isBochaAvailable(): boolean {
	if (!agentcfgServiceSelected("bocha")) return false;
	return hasCredentialSource({ provider: "Bocha", configuredValue: loadConfig().bochaApiKey, environmentValue: undefined });
}
...
export async function searchWithBocha(query: string, options: SearchOptions = {}): Promise<SearchResponse> {
	...
	try {
		response = await agentcfgFetch(BOCHA_SEARCH_URL, {
			method: "POST",
