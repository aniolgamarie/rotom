...
const agentcfgFetch = webFetchFor("exa");
...
export async function callExaMcp(
	...
	const response = await agentcfgFetch(`${EXA_MCP_URL}?tools=${toolName}`, {
		method: "POST",
		headers: {
			"Content-Type": "application/json",
...
export function isExaAvailable(): boolean {
	if (!agentcfgServiceSelected("exa")) return false;
	return true;
}
