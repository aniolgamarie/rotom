import { webBrowserDeclared } from "@agentcfg/pi-runtime/web-browser";
...
async function isGeminiWebOptionallyAvailable(): Promise<boolean> {
  return agentcfgServiceSelected("gemini-web") && webBrowserDeclared();
}
...
async function resolveRedirect(proxyUrl: string, signal?: AbortSignal): Promise<string | null> {
	try {
		const res = await agentcfgFetch(proxyUrl, {
			method: "HEAD",
			redirect: "manual",
