...
const agentcfgFetch = webFetchFor("serpdive");
...
function resolveModel(): SerpdiveModel {
	const raw = loadConfig().serpdiveModel;
...
export function isSerpdiveAvailable(): boolean {
	if (!agentcfgServiceSelected("serpdive")) return false;
...
export async function searchWithSerpdive(query: string, options: SerpdiveSearchOptions = {}): Promise<SearchResponse> {
	...
	try {
		response = await agentcfgFetch(SERPDIVE_API_URL, {
			method: "POST",
			headers: {
