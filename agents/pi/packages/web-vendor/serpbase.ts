...
const agentcfgFetch = webFetchFor("serpbase");
...
export function isSerpBaseAvailable(): boolean {
	if (!agentcfgServiceSelected("serpbase")) return false;
	return hasCredentialSource({ provider: "SerpBase", configuredValue: loadConfig().serpbaseApiKey, environmentValue: undefined });
}
