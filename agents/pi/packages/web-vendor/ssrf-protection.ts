...
function loadConfigRoot(): Record<string, unknown> | null { return agentcfgWebConfig(); }
...
export async function fetchRemoteUrl(
	...
	const fetchImpl = options.fetch ?? agentcfgFetch;
...
// 代理由 agentcfg 显式路线决定；环境变量不能跳过地址检查。
function shouldTrustEnvProxy(_url: URL, _enabled: boolean): boolean { return false; }
