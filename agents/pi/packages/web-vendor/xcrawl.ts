...
const agentcfgFetch = webFetchFor("xcrawl");
...
export function isXcrawlAvailable(): boolean {
	if (!agentcfgServiceSelected("xcrawl")) return false;
	return hasCredentialSource({ provider: "XCrawl", configuredValue: loadConfig().xcrawlApiKey, environmentValue: undefined });
}
...
export async function searchWithXCrawl(query: string, options: SearchOptions = {}): Promise<SearchResponse> {
	...
	try {
		response = await agentcfgFetch(XCRAWL_API_URL, {
			method: "POST",
			headers: {
