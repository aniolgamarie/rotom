...
const agentcfgFetch = webFetchFor("serper");
...
export function isSerperAvailable(): boolean {
	if (!agentcfgServiceSelected("serper")) return false;
	return hasCredentialSource({ provider: "Serper", configuredValue: loadConfig().serperApiKey, environmentValue: undefined });
}
...
export async function searchWithSerper(query: string, options: SearchOptions = {}): Promise<SearchResponse> {
	...
	try {
		response = await agentcfgFetch(SERPER_SEARCH_URL, {
			method: "POST",
