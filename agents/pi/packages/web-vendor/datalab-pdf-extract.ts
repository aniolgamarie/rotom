...
const agentcfgFetch = webFetchFor("datalab");
...
function loadConfig(): DatalabConfig { return agentcfgWebConfig() as DatalabConfig; }
...
export function isDatalabApiAvailable(): boolean {
	if (!agentcfgServiceSelected("datalab")) return false;
...
export function getDatalabApiBase(): string {
	const normalized = normalizeString(agentcfgWebConfig().datalabApiBase)?.replace(
		/\/+$/,
		"",
	);
...
export function getDatalabProcessingLocation(): DatalabProcessingLocation {
	const value =
		normalizeString(agentcfgWebConfig().datalabProcessingLocation) ??
...
export async function extractPDFViaDatalab(
	...
	const mode = options.mode ?? normalizeDatalabMode(agentcfgWebConfig().datalabMode);
...
async function fetchDatalab(
	...
	try {
		return await agentcfgFetch(url, { ...init, redirect: init.redirect ?? "error" });
	} catch (error) {
