...
const agentcfgFetch = webFetchFor("kagi");
...
export function isKagiAvailable(): boolean {
	if (!agentcfgServiceSelected("kagi")) return false;
	return hasCredentialSource({ provider: "Kagi", configuredValue: loadConfig().kagiApiKey, environmentValue: undefined });
}
...
export async function searchWithKagi(query: string, options: KagiSearchOptions = {}): Promise<SearchResponse> {
	...
	try {
		response = await agentcfgFetch(KAGI_SEARCH_URL, {
			method: "POST",
...
export async function extractWithKagi(url: string, signal?: AbortSignal, options: KagiExtractOptions = {}): Promise<ExtractedContent | null> {
	...
	let response: Response;
	try {
		response = await agentcfgFetch(KAGI_EXTRACT_URL, {
			method: "POST",
			headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json", Accept: "application/json" },
