...
const agentcfgFetch = webFetchFor("ollama");
...
export function isOllamaAvailable(): boolean {
	if (!agentcfgServiceSelected("ollama")) return false;
	return hasCredentialSource({ provider: "Ollama", configuredValue: loadConfig().ollamaApiKey, environmentValue: undefined });
}
...
export async function searchWithOllama(query: string, options: OllamaSearchOptions = {}): Promise<SearchResponse> {
	...
	try {
		response = await agentcfgFetch(OLLAMA_SEARCH_URL, {
			method: "POST",
...
export async function extractWithOllama(url: string, signal?: AbortSignal, options: OllamaExtractOptions = {}): Promise<ExtractedContent | null> {
	...
	let response: Response;
	try {
		response = await agentcfgFetch(OLLAMA_FETCH_URL, {
			method: "POST",
			headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json" },
