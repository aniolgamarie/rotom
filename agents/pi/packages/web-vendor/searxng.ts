...
const agentcfgFetch = webFetchFor("searxng");
import { webServiceSelected as agentcfgServiceSelected } from "@agentcfg/pi-runtime/web-binding";
import { webConfig as agentcfgWebConfig, webCredential } from "@agentcfg/pi-runtime/web-config";
...
function getBaseUrl(): string | null {
	return normalizeBaseUrl(loadConfig().searxngBaseUrl);
}
...
function normalizeHeaders(value: unknown): Record<string, string> {
	...
	for (const [key, headerValue] of Object.entries(value as Record<string, unknown>)) {
		...
		const resolved = webCredential(headerValue);
		if (!isValidHeaderValue(resolved)) throw new Error("WEB_HEADER_INVALID");
...
export function isSearXNGAvailable(): boolean {
	if (!agentcfgServiceSelected("searxng")) return false;
	const baseUrl = getBaseUrl();
...
export async function searchWithSearXNG(query: string, options: SearchOptions = {}): Promise<SearchResponse> {
	...
	try {
		...
		const response = await agentcfgFetch(url, {
			method: "GET",
			headers,
