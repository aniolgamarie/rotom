> agentcfg 派生构建：先读 [运行边界](../NOTICE.agentcfg.md)。以下保留上游参考；动态发现、嵌套、调度与脚本工作流在此构建中关闭。
...
## agentcfg external executor v2

The agentcfg build attaches `agentcfg:subagents:external:v2:` to the existing AgentManager.
Operations are `submit_delegate`, `get_delegate_result`, `cancel_delegate`, and `submit_batch`.
All envelopes are closed and bound to the current instance/owner. The manager's existing capacity
includes session, managed-process and external execution. A cancel acknowledgment keeps the slot
until the supervisor proves physical reclamation. Lost submission responses are not retried.
Task Keeper managed scopes cannot invoke this unmetered external backend.
Batch records only reference explicitly submitted runs; this interface adds no independent scheduler.
