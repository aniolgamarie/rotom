import { assertOrdinaryOptions, assertSingleManager, MANAGER_KEY } from "./agentcfg-config.js";
import { attachManaged } from "./agentcfg-managed.js";
/**
...
import { AgentManager, bindSessionBackend, isTopLevelAgent } from "./agent-manager.js";
import { getAgentConversation, getDefaultMaxTurns, getGraceTurns, getRememberAgents, normalizeMaxTurns, resolveEffectiveMaxTurns, resumeAgent, runAgent, SUBAGENT_TOOL_NAMES, setDefaultMaxTurns, setGraceTurns, setRememberAgents, steerAgent } from "./agent-runner.js";
...
export default function (pi: ExtensionAPI) {
  ...
  assertSingleManager();
  bindSessionBackend({ runAgent, resumeAgent });
  ...
  const manager = new AgentManager((record) => {
    if (manager.isControlled(record.id)) { widget.update(); return; }
  ...
  const spawnTopLevel = (piRef: any, ctxRef: any, type: string, prompt: string, options: any) => {
    ...
    assertOrdinaryOptions(safeOptions);
  ...
  const resolveAgentRef = (ref: string): AgentRecord | undefined => {
    ...
    if (byId) return manager.isControlled(byId.id) ? undefined : byId;
    const resolved = manager.resolveMention(ref);
    return resolved?.kind === "live" && !manager.isControlled(resolved.record.id) ? resolved.record : undefined;
  };
  ...
  const registryEntry = {
    ...
    getRecord: (id: string) => {
      ...
      return record !== undefined && !manager.isControlled(id) && isTopLevelAgent(record) ? record : undefined;
    },
  };
  ...
  const detachManaged = attachManaged(manager, pi, () => {
    if (!currentCtx) throw new Error("AGENTCFG_SESSION_NOT_READY");
    return currentCtx;
  });
    ...
    if (!rpcHandle) {
      rpcHandle = registerRpcHandlers({
        ...
        manager: {
          ...
          getRecord: (id) => manager.isControlled(id) ? undefined : manager.getRecord(id),
  ...
  pi.on("input", async (event, ctx) => {
    ...
    if (resolved?.kind === "live") {
      ...
      if (manager.isControlled(record.id)) return { action: "handled" };
  ...
  function setSchedulingEnabled(b: boolean) { if (b) throw new Error("AGENTCFG_SCHEDULING_DISABLED"); schedulingEnabled = false; }
  ...
  function setWorkflowsEnabled(b: boolean) {
    if (b) throw new Error("AGENTCFG_WORKFLOWS_DISABLED");
