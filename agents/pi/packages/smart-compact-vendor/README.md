...
## agentcfg adaptation (in progress)

This package uses `agent_options.smart_compact` and the single configured compaction owner. Model calls use the current constrained ModelRuntime;
managed helper calls fail before sending. State, backups and metrics use the selected instance's pi-home. The project fingerprint uses declared project roots,
without running an unsupervised Git command. Native settings writers direct the user to agentcfg machine overrides.
The remaining tool/configuration integration is still being validated; this package is not yet selected by the build recipes.

Automatic smart compaction uses the `settled` trigger while native automatic compaction is disabled.
The before-compaction hook cancels when no smart result is available, so native summarization does not become a second owner.
Secret scrubbing is mandatory under the repository contract. The fixed schema rejects disabling it.
