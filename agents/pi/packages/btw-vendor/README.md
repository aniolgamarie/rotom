...
The helper uses the current constrained ModelRuntime. Managed task helpers are rejected; request retries are owned outside the provider wrapper.
Configuration comes from `agent_options.btw`; native settings writes direct the user to agentcfg machine overrides. Side-thread history remains instance state.
The upstream browser-opening subprocess still requires adaptation; this package is not yet selected by the build recipes.
