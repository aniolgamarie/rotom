...
import { registerMcpNamespace, unregisterMcpTool, type McpToolOwner } from "@agentcfg/pi-runtime/mcp-tools";
...
function registerNamespaceProxyTool(
  ...
  (input.pi.registerTool as (tool: unknown) => unknown)({
    ...
    renderResult,
    execute: registerMcpNamespace(spec.toolName, spec.serverName, namespaceExecute(
      input.getState,
...
function deactivateStaleNamespaceTools(input: SyncNamespaceProxyToolsInput, nextNames: Set<string>): string[] {
  ...
  for (const stale of staleNames) {
    unregisterMcpTool(stale, input.toolOwner);
