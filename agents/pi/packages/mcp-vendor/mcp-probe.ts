...
async function probe(url: string | URL, strategy: ProbeStrategy, fetch: typeof globalThis.fetch): Promise<ProbeResult> {
...
export async function probeMcpEndpoint(url: string | URL, fetch: typeof globalThis.fetch = globalThis.fetch): Promise<McpProbeResult> {
  const { response: modernResponse, outcome: modernOutcome } = await probe(url, MODERN_STRATEGY, fetch);
  ...
  const { response: postResponse, outcome: postOutcome } = await probe(url, LEGACY_POST_STRATEGY, fetch);
  ...
  const { response: getResponse, outcome: getOutcome } = await probe(url, LEGACY_SSE_STRATEGY, fetch);
