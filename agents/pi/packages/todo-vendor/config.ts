interface GuidanceFields { promptSnippet?: string; promptGuidelines?: string[]; }
function validateGuidanceFields(fields: unknown): GuidanceFields {
  if (fields === undefined) return {};
  if (!fields || typeof fields !== "object" || Array.isArray(fields)
      || Object.keys(fields).some(key => !["promptSnippet", "promptGuidelines"].includes(key))) throw new Error("PI_TODO_CONFIG_INVALID");
  const value = fields as GuidanceFields;
  if (value.promptSnippet !== undefined && typeof value.promptSnippet !== "string"
      || value.promptGuidelines !== undefined && (!Array.isArray(value.promptGuidelines) || value.promptGuidelines.some(item => typeof item !== "string"))) throw new Error("PI_TODO_CONFIG_INVALID");
...
export function loadConfig(): TodoConfig {
	return pluginSettings("pi-todo", "todo") as TodoConfig;
}
