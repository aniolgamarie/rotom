...
import { assertSessionBoundary, requireOrdinaryHelper } from "@agentcfg/pi-runtime/capability-policy";
...
export default function (pi: ExtensionAPI) {
	pi.registerCommand("handoff", {
      ...
      requireOrdinaryHelper(runtime, "handoff");
      await assertSessionBoundary(runtime);
			...
			const result = await ctx.ui.custom<string | null>((tui, theme, _kb, done) => {
				...
				const doGenerate = async () => {
					...
					const response: AssistantMessage = await runtime.models.current.complete(
						ctx.model!,
				...
				doGenerate()
					.then(done)
					.catch((err) => {
						console.error("HANDOFF_GENERATION_FAILED");
						done(null);
					});
